...
public abstract class BaseDevice {
    ...
    protected synchronized void sendCommand(String hexCommand) {
        sendCommandBatch(Arrays.asList(hexCommand));
    }

    protected synchronized void sendCommandBatch(List<String> hexCommands) {
        SessionState sessionState = sessionService.getSessionState();
        for (String hexCommand : hexCommands) {
            byte[] request = prepareCommand(hexCommand, sessionState);
            transportService.sendPackage(request, new ResponseValidator(request));
        }
    }


    protected byte[] prepareCommand(String commandTypeHex, SessionState sessionState) {
        ...
        command[8] = sessionState.getSequenceNumber();
        command[19] = zoneId;
    ...
    public abstract void synchronizeState();
